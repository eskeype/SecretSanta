...
def get_participant_messages(participants):
	...
	for gift_giver, gift_receiver in matching.items():
		participant_messages[gift_giver] = get_message(gift_giver, gift_receiver)
...
def main():
	...
	if len(sys.argv) > 1 && sys.argv[1] == 'test':
		participant_messages = get_test_participant_messages(participants, TEST_MESSAGE_SKELETON.format(1))
	else:
